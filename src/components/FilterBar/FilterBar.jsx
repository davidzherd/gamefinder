...
export function MobileFilters(
  { currentPlatforms, currentGaneres, filterAction, closeModal },
  ...props
) {
  return (
    <div className="mobile-filter-wrap">
      ...
      <main className="mobile-filters">
              ...
              <button
                className={
                  currentPlatforms.includes(platform.id)
                    ? "ganre-btn selected-filter"
                    : "ganre-btn"
                }
                onClick={() => filterAction(platform.id, "platform")}
                key={platform.name}
              >
                {platform.name}
              </button>
        ...
        <div className=" ml-1">
          <h2>Ganres</h2>
          <div>
            {config.ganres.map((ganre) => (
              <button
                className={
                  currentGaneres.includes(ganre.name)
                    ? "ganre-btn selected-filter"
                    : "ganre-btn"
                }
                onClick={() => filterAction(ganre.name, "ganre")}
                key={ganre.name}
              >
                {ganre.name}
              </button>
...
function FilterBar(
  { currentPlatforms, currentGaneres, filterAction },
  ...props
) {
  ...
  const mobile = (
    <div className="margin-1rem mobile-layout-column">
      <Button style="fill-btn" action={() => setModalOpen(true)}>
        Apply Filters
      </Button>
      {allFilters.length > 0 ? (
        <h2>Applied filters</h2>
      ) : (
        <h2>You didn't apply any filters!</h2>
      )}
  ...
  return componentType ? desktop : mobile;
}
